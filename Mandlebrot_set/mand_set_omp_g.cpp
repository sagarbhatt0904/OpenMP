...
{		
	...
	#pragma omp parallel for schedule(guided, 50) num_threads(numthd[k])
	 for(int i=0; i<N; i++)
	{
		for(int j=0; j<N; j++)
		{
	...
	#pragma omp parallel for reduction(+:area) schedule(guided, 50) num_threads(numthd[k])
	for (int i = 0; i < N; ++i)
	{
		for (int j = 0; j < N; ++j)
		{
	...
	// cout<<endl<<"Time with guided omp: "<<time_s[k]<<endl<<"Area of the mandlebrot set ="<<area<<endl;
}

ofstream fout("mand_perform_g.csv");
for (int i = 0; i < 12; ++i)
{	
