...
{		
	...
	#pragma omp parallel for schedule(dynamic, 50) num_threads(numthd[k])
	 for(int i=0; i<N; i++)
	{
		for(int j=0; j<N; j++)
		{
	...
	#pragma omp parallel for reduction(+:area) schedule(dynamic, 50) num_threads(numthd[k])
	for (int i = 0; i < N; ++i)
	{
		for (int j = 0; j < N; ++j)
		{
	...
	// cout<<endl<<"Time with dynamic omp: "<<time_s[k]<<endl<<"Area of the mandlebrot set ="<<area<<endl;
}

ofstream fout("mand_perform_d.csv");
for (int i = 0; i < 12; ++i)
{	
